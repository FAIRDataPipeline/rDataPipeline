...
# census geography (alternatively a population dataset with preset age classes 
# can be used) in the structure: 1st column - Geographical identifier, 
...
# desired (i.e. 5-year age classes) the structure of this new age class must be 
# set as a vector called: ageclasses
...
# and 90+ ageclass
dz_populations_full <- h5read(file = h5filename, 
...
age_class_datazone <- matrix(data = 0, ncol = num_ageclasses,
                             nrow = nrow(dz_populations_full))
...
for(i in seq_along(ageclasses)) {
  ...
  age_class_datazone[,i] <- dz_populations_full %>% 
    rename(AGE90 = "AGE90+") %>% 
    select(all_of(columns)) %>% 
    rowSums()
}
age_class_datazone <- cbind.data.frame(dz_populations_full$DataZone2011Code,
                                       age_class_datazone)
colnames(age_class_datazone) <- c("DataZone", ageclasses)
...
# Find proprotion of each census geography in each grid cell

# Read in datazones shapefile and check for non-intersecting geometries
shape <- st_read(dsn = "data-raw/shapefiles/SG_DataZone_Bdry_2011.shp")
...
datazones %<>% filter(DataZone %in% dz_populations_full$DataZone2011Code) 
...
grid_50 <- st_make_grid(st_as_sfc(st_bbox(datazones)), 
...
postcode %>% mutate(nchar = nchar(Postcode)) %$% nchar %>% unique
postcode %>% mutate(nchar = nchar(Postcode)) %>% 
  filter(nchar==9) %$% Postcode %>% unique
...
dz_grid_postcode <- st_join(postcode, intersected_grid_contain)
...
# g1 <- ggplot(postcode %>% filter(DZ11 == "S01007285")) +
#   geom_sf() +
#   geom_sf_text(aes(label = Postcode), size = 5)
# 
# g2 <- ggplot(postcode %>% filter(DZ11 == "S01007285")) +
#   geom_sf(data = postcode %>% filter(DZ11 == "S01007285")) +
#   geom_sf(data = postcode %>% filter(Postcode == "PA75 6NUA"), fill = "red") +
#   geom_sf(data = postcode %>% filter(Postcode == "PA75 6NUB"), fill = "blue") +
#   geom_sf_text(aes(label = Postcode), size = 5) + 
#   coord_sf(xlim = c(147000, 153000), ylim = c(755000, 759000), expand = FALSE)
# 
# ggsave('map.pdf', g1, width = 20, height = 10)
# ggsave('map2.pdf', g2, width = 20, height = 10)
# 
# postcode %>% filter(Postcode == "PA75 6NUA")
# g3 <- ggplot(postcode %>% filter(DZ11 == "S01007286")) +
#   geom_sf() +
#   geom_sf_text(aes(label = Postcode), size = 5) +
#   geom_sf(data = postcode %>% filter(Postcode == "PA75 6NUA"), fill = "red")
# ggsave('map3.pdf', g3, width = 10, height = 20)
# 
# g4 <- ggplot() +
#   geom_sf(data = postcode %>% filter(DZ11 == "S01007286"),
#           fill = "blue", alpha = 0.3) +
#   geom_sf(data = datazones %>% filter(DataZone == "S01007286"), 
#           fill = "red", alpha = 0.3)
# ggsave('map4.pdf', g4, width = 10, height = 20)
...
# postcodes in each datazone component 
...
# postcode_inout[which(is.na(postcode_inout$grid_id)==TRUE),]
# postcode_inout[which(postcode_inout$DZ11=="S01007285"),]
...
# datazones
wide_new_table <- pivot_wider(combined_areas, names_from = "DZ11", 
...
grid_populations <- matrix(data = 0, nrow = nrow(wide_new_table_pop_sizes),
...
# which is in each grid cell (I'm sure theres a more efficient way t do this 
# but not sure what it is!)
for(j in seq_along(ageclasses)) {
  for(i in seq_len(ncol(wide_new_table_pop_sizes))) {
    cat("\rageclass", j, "of", length(ageclasses), 
        "; datazone", i, "of", ncol(wide_new_table_pop_sizes), "...")
    ...
    in_gridcell_pops <- age_class_datazone %>% 
      filter(DataZone == colnames(in_gridcell)) %>% 
      select(all_of(tag)) %>% 
      as.numeric()
    
    rounded_pops <- round(in_gridcell*in_gridcell_pops)
    ...
    # Check that rounded population size matches datazone population size 
    # in age class
    assertthat::assert_that(sum(rounded_pops) == sum(in_gridcell_pops))
  }
  ...
  assertthat::assert_that(!any(is.na(wide_new_table_pop_sizes)))
  
  # Add populations from each datazone together to find total population for 
  # each grid cell
  grid_populations[, tag] <- rowSums(wide_new_table_pop_sizes)
}

# Save to hdf file
h5write(grid_populations, file = h5filename, 
        name = "griddata/5year_age_class_scotland_10kmgrid_postcode")
