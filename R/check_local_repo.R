...
check_local_repo <- function(path) {
  ...
  if(length(tmp$staged) != 0)
    usethis::ui_oops(paste("Local repository is unclean, please check staged files:",
                           usethis::ui_value(tmp$staged)))

  if(length(tmp$unstaged) != 0)
    usethis::ui_stop(paste("Local repository is unclean, please check unstaged files:",
                           usethis::ui_value(tmp$unstaged)))

  if(length(tmp$untracked) != 0)
    usethis::ui_stop(paste("Local repository is unclean, please check untracked files:",
