...
dz2lower <- function(dat, convert_to, conversion_table) {
  ...
  assertthat::assert_that(all(dat$LAGcode %in%
                                conversion_table$LAGcode))

  # Convert datazones
  regex <- paste0("LAG|", toupper(convert_to))
  ...
  # Check that LAGcode isn't duplicated in the conversion table
  assertthat::assert_that(!any(duplicated(subset.table$LAGcode)))

  output <- dat %>%
    dplyr::full_join(subset.table, by = "LAGcode") %>%
