...
process_ons_demographics <- function (sourcefile, h5filename){
  ...
  output_area_sf <- file.path("data-raw", "outputarea_shapefile", "Output_Areas__December_2011__Boundaries_EW_BFC.shp")
  ...
  subgrp.names <- c("total", "1year", "5year", "10year", 
                    "sg_deaths_scheme")
  age.classes <- list("total", 0:90, seq(0, 90, 5), 
                      seq(0, 90, 10), c(0, 1, 15, 45, 65, 75, 85))
  ...
  OA_EW_LA <- readr::read_csv("data-raw/england_lookup/output_to_ward_to_LA.csv",
    ...
    dplyr::rename(OAcode = OA11CD, EWcode = WD19CD, EWname = WD19NM, 
  ...
  OA_LSOA_MSOA_LA <- readr::read_csv("data-raw/england_lookup/output_to_LSOA_MSOA_to_LA.csv",
                                     col_types = cols(.default = "c"))  %>%
    dplyr::rename(OAcode = OA11CD, LSOAcode = LSOA11CD, LSOAname = LSOA11NM, 
  ...
  original.dat <- lapply(seq_along(sourcefile), function(k) {
    ...
    header_new <- header_new %>% 
      ...
      gsub("2011outputarea", "OAcode",., fixed=TRUE)
    ...
    for (j in seq_along(subgrp.names)) {
      ...
      for (i in seq_along(grp.names)) {
        cat(paste0("\rProcessing ", j, "/", length(subgrp.names), 
                   ": ", i, "/", length(grp.names), "..."))
        if (grp.names[i] %in% 
            c("OA", "EW", "LA", "LSOA", "MSOA", "CCG", "STP", "UA","LHB")) {
        ...
        #                            oa_subdivisions = oa_subdivisions[[grp.names[i]]])
        }
        else {
          stop("OMG! - grpnames")
        }
