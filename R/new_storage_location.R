#' new_storage_location
#'
#' Upload information to the `storage_location` table in the data registry
#'
#' @param path a `string` specifying the path from the `storage_root`
#' URI to the item location, which when appended to `storage_root` URI produces
#' a complete URL
#' *e.g.* "master/SCRC/human/infection/SARS-CoV-2/latent-period/0.1.0.toml"
#' @param hash a `string` specifying the SHA1 hash of the file stored in
#' `storage_location` *e.g.* "7c0e14caec08674d7d4e52c305cb4320babaf90f"
#' @param storage_root_id a `string` specifying the API URL of the
#' associated `storage_root` table
#' *e.g.* "https://data.scrc.uk/api/storage_root/14/"
#' @param key API token from data.scrc.uk
#'
#' @export
#'
new_storage_location <- function(path,
                                 hash,
                                 storage_root_id,
                                 key) {

  post_data(table = "storage_location",
            data = list(path = path,
                        hash = hash,
                        storage_root = storage_root_id),
            key)
}
