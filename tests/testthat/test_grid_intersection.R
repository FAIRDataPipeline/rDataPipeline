...
test_that("grid_intersection() should fail if shapefile not in metres", {
  # Grid intersection takes a gridsize in km and assumes the shapefile uses
  # a CRS projected in metres - we should not be silently producing an invalid
...
test_that("grid_intersection() should return only parts of grid which intersect with features", {
  ...
  covering_square_coords <- rbind(c(0, 0), c(map_extent, 0),
                                  c(map_extent, map_extent), c(0, map_extent),
                                  c(0, 0))
  ...
  testthat::expect_false(all(sf::st_covers(result$subdivisions, empty_area,
                                           sparse = FALSE)))
})
