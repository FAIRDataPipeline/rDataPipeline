...
test_that("grid_intersection() should fail if shapefile not in metres", {
  # Grid intersection takes a gridsize in km and assumes the shapefile uses a
  # CRS projected in metres - we should not be silently producing an invalid
  # result when the CRS uses a different unit
  triangle <- sf::st_sfc(sf::st_polygon(
...
test_that("grid_intersection() should produce expected number of cells", {
  ...
  square <- sf::st_sfc(sf::st_polygon(
    list(rbind(c(0,0), c(max_dim,0), c(max_dim,max_dim),
               c(0,max_dim), c(0,0)))))
...
# Returned grid should be clipped to the extent of the input shapefile /
# features (i.e. return an actual intersection):
...
test_that("grid_intersection() should return only parts of grid which intersect with features", {
  ...
  covering_square_coords <- rbind(c(0, 0), c(map_extent, 0),
                                  c(map_extent, map_extent),
                                  c(0, map_extent), c(0, 0))
  empty_area <- sf::st_sfc(sf::st_polygon(
    list(covering_square_coords, square_coords, triangle_coords)))
  empty_area <- sf::st_sf(empty_area, crs = DEFAULT_CRS)

  testthat::expect_false(all(sf::st_covers(result$subdivisions,
                                           empty_area, sparse = FALSE)))
})
...
test_that("grid_intersection() should produce cells of expected size at edges of irregular features", {
  ...
  triangle <- sf::st_sfc(sf::st_polygon(
    list(rbind(c(0,0), c(max_dim,0), c(max_dim,max_dim), c(0,0)))))
  ...
  testthat::expect_equal(sf::st_area(result$subdivisions$grids[1]),
                         expected_area)
})
